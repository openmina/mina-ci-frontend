...
export class ReportingEffects extends MinaBaseEffect<ReportingActions> {
  ...
  constructor(private actions$: Actions,
    ...
    this.getReports$ = createEffect(() => this.actions$.pipe(
      ofType(REPORTING_GET_REPORTS, REPORTING_CLOSE),
      this.latestActionState<ReportingGetReports | ReportingClose>(),
      switchMap(({ action }) =>
        action.type === REPORTING_CLOSE ? EMPTY : this.reportingService.getReports()
          .pipe(
